    ...
    server.route(
      ...
      , handler: function (req, reply) {
          var playerId = parseUUID(req.payload.data.attributes.player_id)
            , p = BanModel(req.app.server)
            .forge({ 'player_id': playerId })
            .fetch()
            .then(function (ban) {
              if (ban) {
      ...
      , handler: function (req, reply) {
          var punishment
            ...
            .then(function (ban) {
              ...
              punishment = ban.clone().attributes

              req.log.debug(punishment, 'ban')
            ...
            .then(function (record) {
              reply({ meta: { id: record.get('id'), type: 'player-ban-record' } }).code(200)
            })
            .catch(reply)
        }
      , config:
