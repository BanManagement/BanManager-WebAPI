...
{ PlayerReport:
  ...
  , locations:
    { async resolve(report, args, { state }) {
        ...
        const playerLocation = find(locationResults, { 'player_id': parse(report.player.id, new Buffer(16)) })
        const actorLocation = find(locationResults, { 'player_id': parse(report.actor.id, new Buffer(16)) })
        const locations = {
          player: {
            x: playerLocation.x,
            y: playerLocation.y,
